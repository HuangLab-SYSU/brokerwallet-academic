...
/**
 * BrokerChain token module configuration. / BrokerChain token 模块配置。
 *
 * The network address and chain connection configuration have been migrated to {@link ChainConfig}; only the token is retained here. / 网络地址和链连接配置已经迁移到 {@link ChainConfig}；这里仅保留 token
 * Module's display, timeout and retry parameters, and provides compatibility with a few old fields. / 模块的显示、超时和重试参数，并提供少量旧字段兼容。
 */
public final class TokenConfig {
    ...
    /** balanceOf alone uses a shorter timeout, and after failure the caller can retry or fall back to raw RPC. / balanceOf 单独使用较短超时，失败后调用方可重试或回退到 raw RPC。 */
    ...
    /** Build the {@code gettx2?acc=} URL; {@code account} may include or omit {@code 0x}. / ?? {@code gettx2?acc=} URL?{@code account} ?????? {@code 0x}? */
